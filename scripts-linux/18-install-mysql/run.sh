...
APT_PKG="mysql-server"; VERIFY_CMD='mysql --version'; INSTALLED_MARK="$ROOT/.installed/18.ok"
verify_installed() { bash -c "$VERIFY_CMD" >/dev/null 2>&1; }
verb_install() {
  log_info "[18] Starting MySQL Server installer"
  if verify_installed; then log_ok "[18] Already installed"; mkdir -p "$ROOT/.installed"; touch "$INSTALLED_MARK"; return 0; fi
  if ! is_debian_family || ! is_apt_available; then log_err "[18] apt not available"; return 1; fi
  log_info "[18] Installing via apt: $APT_PKG"
  sudo apt-get update -y >/dev/null 2>&1 || true
  if sudo apt-get install -y $APT_PKG; then
    log_ok "[18] Installed"; mkdir -p "$ROOT/.installed"; touch "$INSTALLED_MARK"; return 0
  fi
...
case "${1:-install}" in install) verb_install;; check) verb_check;; repair) verb_repair;; uninstall) verb_uninstall;; *) log_err "[18] Unknown verb: $1"; exit 2;; esac
